...
# Setting up facial tracking
face_cascade = cv2.CascadeClassifier("./haarcascades/haarcascade_frontalface_default.xml")
eye_cascade = cv2.CascadeClassifier("./haarcascades/haarcascade_eye.xml")
def draw_rectangle(coordinates, image, color=(0,0,255)):
    ''' Given coordinates in the form x, y, w, h, draw the rectangle '''
    x, y, w, h = coordinates
...
while exit_flag:
    ...
    faces = face_cascade.detectMultiScale(imageGray, face_scale_factor, face_min_frames)
    for face in faces:
        draw_rectangle(face, image)
        x, y, w, h = face
        roi_gray = imageGray[y:y+h, x:x+w]
        roi_color = image[y:y+h, x:x+w]
        eyes = eye_cascade.detectMultiScale(roi_gray)  # , eye_scale_factor, eye_min_frames)
        for eye in eyes:
            # eye = np.array(eye,dtype=int) + np.array([x, y, 0, 0])
            draw_rectangle(eye, roi_color, color=(0, 255, 0))
